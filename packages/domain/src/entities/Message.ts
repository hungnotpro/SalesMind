...
export function validateIncomingMessage(msg: unknown): IncomingMessage {
  ...
  if (typeof m.externalMessageId !== 'string' || !m.externalMessageId.trim()) {
    throw new Error('External message ID is required');
  }
  ...
  if (m.receivedAt) {
    const receivedAt = new Date(m.receivedAt as string);
    if (isNaN(receivedAt.getTime())) {
      throw new Error('Invalid received_at date');
    }
  }
  ...
  return {
    ...
    sender: {
      name: (m.sender as Record<string, string>)?.name?.trim(),
      phone: (m.sender as Record<string, string>)?.phone?.trim()
    },
...
export function createMessage(incoming: IncomingMessage, id: string): Message {
  ...
  return {
    ...
    metadataJson: incoming.metadata ? JSON.stringify(incoming.metadata) : undefined,
